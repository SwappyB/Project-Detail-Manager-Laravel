...
@section('content')
    @if(count($data['customers']) > 0)  
      ...
      <div style="padding-bottom:60px;">
        <button class="btn btn-danger btn-lg float-right" style="margin-left:10px" onclick="location.href='{{ url('/customer') }}'">Go Back</button>
        <h1 class="float-left" style="color:#212529">{{$data['customers'][0]->name}}</h1>
        <a href="{{$data['customers'][0]->cid}}/edit" class="btn btn-success btn-lg float-right">Edit</a>
    </div>
    <hr>

    <div class="row">
      <div class="col-2">
          <p class="showProject2">Industry</p>
      </div>
      <div class="col-2">
          <p class="showProject">{{$data['customers'][0]->industry}}</p>
      </div>
    </div>
    <hr>
    <div class="row">
        ...
        <div class="col-10">
            <p class="showProject">{{$data['customers'][0]->description}}</p>
        </div>
    </div>
    <hr>
    <div class="row">
        ...
        <div class="col-4" style="border-right:1px solid rgba(0, 0, 0, 0.1)">
            <p class="showProject">{{$data['customers'][0]->email}}</p>
        </div>
        ...
        <div class="col-4">
            <p class="showProject">{{$data['customers'][0]->phoneNumber}}</p>
        </div>
    </div>
    <hr>
    <div class="row">
        <div class="col-2">
            <p class="showProject2">Address</p>
        </div>
        <div class="col-10">
            <p class="showProject">{{$data['customers'][0]->address}}</p>
        </div>
    </div>
    <hr>
    <div class="row">
        ...
        <div class="col-4" style="border-right:1px solid rgba(0, 0, 0, 0.1)">
            <p class="showProject">{{$data['customers'][0]->created_at}}</p>
        </div>
        ...
        <div class="col-4">
            <p class="showProject">{{$data['customers'][0]->updated_at}} </p>
        </div>
    </div>
    <hr>
    ...
    <div class="container" style="padding-bottom:150px;">
        <form action="{{ action('CustomersController@destroy', $data['customers'][0]->cid ) }}" method="POST">
          <input name="_token" type="hidden" value="{{ csrf_token() }}"/>
          <input type="hidden" name="_method" value="delete" />
          <input type="submit" onclick="return confirm('Customer details will be deleted!')" value="Delete" class="btn btn-danger btn-lg float-right">
        </form>
    </div>

    @else
    <div class="alert alert-warning" role="alert">
      <h5>Customer Id is Invalid! <a href="/customer" class="alert-link">Go Back</a></h5>
    </div>
    @endif
